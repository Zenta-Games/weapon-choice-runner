...
public class uTransformEditor : Editor {
    ...
    private void DrawUtilities()
    {
        ...
        if (GUILayout.Button("Copy Position", uEditorUtils.uEditorSkin.button))
        {
            if (TransformClipboard.copied)
            {
                if (EditorUtility.DisplayDialog("Warning", "Already copied some properties,are you sure ?", "Yes", "No"))
                {
                    TransformClipboard.copied = true;

                    TransformClipboard.position = positionProperty.vector3Value;

                    TransformClipboard.type = TransformClipboard.Type.Position;
                }
            }
            else
            {
        ...
        if (GUILayout.Button("Copy Rotation", uEditorUtils.uEditorSkin.button))
        {
            if (TransformClipboard.copied)
            {
                if (EditorUtility.DisplayDialog("Warning", "Already copied some properties,are you sure ?", "Yes", "No"))
                {
                    ...
                    TransformClipboard.rotation = rotationProperty.quaternionValue;

                    TransformClipboard.type = TransformClipboard.Type.Rotation;
                }
            }
            else
            {
        ...
        if (GUILayout.Button("Copy Scale", uEditorUtils.uEditorSkin.button))
        {
            if (TransformClipboard.copied)
            {
                if (EditorUtility.DisplayDialog("Warning", "Already copied some properties,are you sure ?", "Yes", "No"))
                {
                    ...
                    TransformClipboard.scale = scaleProperty.vector3Value;

                    TransformClipboard.type = TransformClipboard.Type.Scale;
                }
            }
            else
            {
        ...
        if (GUILayout.Button("Copy Transform", uEditorUtils.uEditorSkin.button))
        {
            if (TransformClipboard.copied)
            {
                if (EditorUtility.DisplayDialog("Warning", "Already copied some properties,are you sure ?", "Yes", "No"))
                {
                    ...
                    TransformClipboard.type = TransformClipboard.Type.Transform;
                }
            }
            else
            {
        ...
        if (TransformClipboard.copied)
        {
            if (GUILayout.Button("Paste Transform", uEditorUtils.uEditorSkin.button))
            {
                TransformClipboard.copied = false;

                switch (TransformClipboard.type)
                {
                    case TransformClipboard.Type.Position:
                        positionProperty.vector3Value = TransformClipboard.position;
                        break;
                    case TransformClipboard.Type.Rotation:
                        rotationProperty.quaternionValue = TransformClipboard.rotation;
                        break;
                    case TransformClipboard.Type.Scale:
                        scaleProperty.vector3Value = TransformClipboard.scale;
                        break;
                    case TransformClipboard.Type.Transform:
                        positionProperty.vector3Value = TransformClipboard.position;
                        rotationProperty.quaternionValue = TransformClipboard.rotation;
                        scaleProperty.vector3Value = TransformClipboard.scale;
                        break;
                ...
                TransformClipboard.type = TransformClipboard.Type.None;
            }
